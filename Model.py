...
class FE_NGCF(GeneralRecommender): # dim: 64+1500+1 -> 64

    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(FE_NGCF, self).__init__(config, dataset)
        ...
        self.txt_embedding_size = config["embedding_size"] # 1500 or 1800
        ...
        self.price_item_embedding = torch.nn.Embedding.from_pretrained(
            torch.from_numpy(dataset.get_preload_weight(
                'iid_price').astype(np.float32))
        )

        self.fc = nn.Sequential(
            nn.Linear(self.txt_embedding_size+64+1, 512),
            nn.ReLU(),
            nn.Linear(512, 128),
            nn.ReLU(),
